...
from guardrails_ai.presidio_gliner_pii.main import (
    PresidioGlinerPII,
    InferenceInput as InputRequest,
    InferenceOutput as OutputResponse,
)
from models_host.base_inference_spec import BaseInferenceSpec


class InferenceSpec(BaseInferenceSpec):
    model: Optional[PresidioGlinerPII] = None
    model_name = "presidio-gliner-pii"

    def load(self):
        self.model = PresidioGlinerPII(
            use_local=True,
            entities=[
                "CREDIT_CARD",
                "CRYPTO",
    ...
    def infer(self, text: str, entities: List[str]) -> OutputResponse:
        ...
        model = cast(PresidioGlinerPII, self.model)
