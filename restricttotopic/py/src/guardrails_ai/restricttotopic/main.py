...
class RestrictToTopic(Validator):
    ...
    **Key Properties**
    ...
    def get_topics_zero_shot(self, text: str, candidate_topics: List[str]) -> List[str]:
        """Finds the topics in the input text using only the zero-shot model.
    ...
    def validate(
        ...
        if len(all_topics) == 0:
            raise ValueError("There must be at least one valid or invalid topic.")
        ...
        if len(valid_topics) > 0 and not valid_topics_found:
            return FailResult(
                ...
                error_spans=[
                    ErrorSpan(
                        start=0, end=len(value), reason="No valid topic was found."
                    )
                ],
            )
        return PassResult()
        ...
        request_body = {
            "inputs": [
                ...
                },
                {
                    ...
                    "shape": [
                        len(model_input["valid_topics"])
                        + len(model_input["invalid_topics"])
                    ],
                    "data": list(model_input["valid_topics"])
                    + list(model_input["invalid_topics"]),
                    "datatype": "BYTES",
                },
                {
