...
from .resources import (
    KNOWN_ATTACKS,
    get_tokenizer_and_model_by_path,
    get_pipeline_by_path,
)
    ...
    **Key Properties**
        ...
        **kwargs,
        ...
        if self.use_local:
            if not model_path_override:
                ...
                # Saturation:
                self.saturation_attack_detector = PromptSaturationDetectorV3(
                    device=torch.device(device), model_path_override=model_path_override
                )
                # Known attacks:
                embedding_tokenizer, embedding_model = get_tokenizer_and_model_by_path(
                    model_path_override, "embedding", AutoTokenizer, AutoModel
                )
        ...
        self.saturation_attack_scales = (
            DetectJailbreak.DEFAULT_SATURATION_ATTACK_SCALE_FACTORS
        )
    ...
    def _predict_and_remap(
        ...
        for pred in predictions:
            ...
            assert (
                pred[label_field] in {safe_case, fail_case} and 0.0 <= old_score <= 1.0
            )
            if is_safe:
    ...
    def predict_jailbreak(
        ...
        if reduction_function is None:
            return [
                {"known_attack": known, "saturation_attack": sat, "other_attack": pred}
    ...
    def validate(
        ...
        if failed_prompts:
            ...
            for txt, score in zip(failed_prompts, failed_scores):
                failure_message += f'\n"{txt}" (Score: {score})'
            return FailResult(error_message=failure_message)
        return PassResult()
