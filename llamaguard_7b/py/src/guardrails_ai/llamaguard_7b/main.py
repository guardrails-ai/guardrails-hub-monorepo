    ...
    **Key Properties**
    ...
    def _inference_local(self, model_input: str):
        raise NotImplementedError(
            "Local inference is not supported for LlamaGuard7B validator."
        )

    def _inference_remote(self, model_input: str) -> Any:
        ...
        if not self.validation_endpoint:
            raise RuntimeError(
                "validation_endpoint must be set to run remote inference."
            )
        ...
        if status != 200:
            ...
            raise ValueError(
                f"Failed to get valid response from Llamaguard-7B model. Status: {status}. Detail: {detail}"
            )
